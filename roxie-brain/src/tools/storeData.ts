...
import matchOrCreateCategory from "../helpers/category-matcher";
...
const storeData = new DynamicStructuredTool({
  ...
  func: async ({ content, data, category }) => {
    ...
    try {
      ...
      console.log(`💾 Store Data category resolved: "${matched.name}" (id: ${matched.id}, new: ${matched.isNew})`)
      ...
      console.log(`💾 Store Data embedding generated ${embedding.length} dimensions ✅`)
      ...
      console.log(`💾 Store Data stored successfully — id: ${rows[0].id}, category: "${matched.name}" ✅`)
