...
export const findUsers = tool(
  async (input: PropType) => {
    console.log('Find Users: ', input);
    let usersList = [];

    if (input?.name) {
      const user = await User.find({ name: input?.name?.toLowerCase() })
      usersList?.push(user)
    } else if (input?.location) {
      const user = await User.find({ location: input?.location?.toLowerCase() })
      usersList?.push(user)
    } else if (input?.relation) {
      const user = await User.find({ relation: input?.relation?.toLowerCase() })
      usersList?.push(user)
    } else if (input?.phone_number) {
      const user = await User.find({ phone_number: input?.phone_number })
      usersList?.push(user)
    } else if (input?.date_of_birth) {
      const user = await User.find({ date_of_birth: input?.date_of_birth?.toLowerCase() })
      usersList?.push(user)
    } else if (input?.occupation) {
      const user = await User.find({ occupation: input?.occupation?.toLowerCase() })
      usersList?.push(user)
  ...
  },
  {
    name: "findUser",
    description:
      "Use this tool to search for a specific person in the user database using known attributes such as their name, location, relation type, phone number, date of birth, or occupation. This is useful when you need to verify if a person already exists before performing any update or to fetch their details for contextual purposes.",
    schema: z.object({
      name: z
        .string()
        .optional()
